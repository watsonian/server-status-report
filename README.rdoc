...
That will save the script as a file named *status.rb*. The script itself is written in Ruby to take advantage of the [ERB templating system](http://ruby-doc.org/stdlib/libdoc/erb/rdoc/classes/ERB.html) to produce the HTML content.
...
You can change the subject to whatever you want and just replace *some@address.com* with the address you want the email sent to. If you want to CC the email to more email addresses you can use the *-c* flag and pass it a comma separated list of email addresses. Running it once per hour is probably not a bad idea, but you can run it as frequently or infrequently as you like!
